...
@Component({
  selector: 'app-payment-search-provider',
  templateUrl: './search-provider-invoices.component.html',
})
export class SearchProviderInvoicesComponent implements OnInit {
  doClean = false;
  ...
  changedProviderSearch(): void {
    console.log('providerSearch', this.data.provider !== null && this.data.provider !== '');
    if (this.data.provider !== null && this.data.provider !== '') {
      this.filteredProviderNames = this.providerNames.filter((item) => item.includes(this.data.provider));
      console.log('filteredCompanyNames', this.filteredProviderNames);
    } else {
      this.filteredProviderNames = this.providerNames;
      console.log('filteredCompanyNames', this.filteredProviderNames);
    }
